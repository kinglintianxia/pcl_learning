...
int main(int argc, char **argv)
{
    ...
    if (rail_std_file.find("csv") != std::string::npos)
    {
        ...
        while (!infile.eof())
        {
            ...
            if (line[0] != '#' && line[0] != 'P')
            {
                ...
                if (pos != -1)
                {
                    ...
                    rail_pts.push_back(RailPoint(px / 1000.0, py / 1000.0));
                    rail_pairs.push_back(std::make_pair<double, double>(px / 1000.0 / 1000.0, py));
                    // Write out.
                    outfile << px << "," << py << std::endl;
                }
            }
        }
        ...
        {
            cloud->points[i].x = rail_pairs[i].first;
            cloud->points[i].y = rail_pairs[i].second;
            // cloud->points[i].z = 0;
        }

        pcl::io::savePCDFileASCII("rail_std_600pts.pcd", *cloud);
    ...
    else if (rail_std_file.find("pcd") != std::string::npos)
    {
        ...
        for (size_t i = 1; i < cloud->size(); i++)
        {
            ...
            // cloud->points[i].z = 0;
            // out csv file
            outfile << cloud->points[i].x*1000.0 << "," << cloud->points[i].y*1000.0 << std::endl;
        }
        outfile.close();
    }
    else
