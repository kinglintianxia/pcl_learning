...
int main(int argc, char **argv)
{
    ...
    // ... and weight the 'curvature' dimension so that it is balanced against x, y.
    float alpha[3] = {0.5, 0.5, 1.0};
    ...
    // icp.setMaxCorrespondenceDistance(0.01); // 2cm
    ...
    viewer.addPointCloudNormals<PointT, PointNT>(icp_std_cloud_ptr, std_cloud_normal, 1, 0.005, "icp_std_cloud_normals", v1);
    // ICP cloud in red.
    ...
    pcl::visualization::PointCloudColorHandlerGenericField<PointNormalT> normal_cloud_color_icp_h(icp_gocator_normal_ptr, "curvature");
    ...
    while (!viewer.wasStopped() && cnt < pcd_vec.size())
    {
        ...
        if (next_iteration)
        {
            ...
            std::string normal_id = "icp_gocator_cloud_normals" + std::to_string(cnt);
            viewer.addPointCloudNormals<pcl::PointNormal>(icp_gocator_normal_ptr, 1, 0.01, normal_id, v1);
            ...
            //     viewer.addLine<PointNormalT>(model_point, model_point, 0, 255, 0, ss_line, v2);
            // }
            //
